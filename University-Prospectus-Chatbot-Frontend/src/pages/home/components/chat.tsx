          ...
          return (
            <div key={message.id} className="flex flex-col space-y-6 animate-slide-in-up" style={{animationDelay: `${index * 0.15}s`, opacity: 0, animationFillMode: 'forwards'}}>
              {message.user && (
                <div className="flex gap-4 justify-end items-start">
                  <div className="relative group max-w-[80%]">
                    {/* Glow effect */}
                    <div className="absolute inset-0 bg-gradient-to-r from-[#800020] to-[#600018] rounded-2xl blur-xl opacity-30 group-hover:opacity-50 transition-opacity duration-300"></div>
                    
                    {/* Message bubble */}
                    <div className="relative bg-gradient-to-br from-[#800020] to-[#600018] text-white rounded-2xl rounded-tr-md px-7 py-5 shadow-2xl shadow-[#800020]/30 transform hover:scale-[1.02] transition-all duration-300 border border-[#A00030]/30">
                      {/* Shine effect */}
              ...
              <div className="flex gap-4 justify-start items-start">
                <div className='relative flex-shrink-0'>
                  {/* Pulsing glow */}
                  <div className='absolute inset-0 bg-[#800020] rounded-full blur-lg opacity-40 animate-pulse'></div>
                  <div className='relative flex justify-center items-center rounded-full bg-gradient-to-br from-[#800020] to-[#600018] w-12 h-12 shadow-2xl border-2 border-[#A00030] transform hover:scale-110 hover:rotate-12 transition-all duration-300'>
                    <TbRobot className="text-white w-6 h-6" />
                  </div>
                </div>
                
                <div className="relative group max-w-[80%]">
                  {/* Subtle glow */}
                  <div className="absolute inset-0 bg-[#800020] rounded-2xl blur-xl opacity-10 group-hover:opacity-20 transition-opacity duration-300"></div>
                  
                  {/* Message bubble */}
                  <div className="relative bg-white dark:bg-gray-900 text-gray-900 dark:text-white rounded-2xl rounded-tl-md px-7 py-5 shadow-2xl shadow-[#800020]/20 border-2 border-gray-100 dark:border-gray-800 transform hover:scale-[1.02] transition-all duration-300">
                    {/* Shine effect */}
                    ...
                    {message.bot === '' && isPendingAnswer ? (
                      <div className="flex items-center gap-3">
                        <BeatLoader size={8} color='#800020' />
